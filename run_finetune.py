...
def main():
    ...
    if args.do_ensemble_pred and args.local_rank in [-1, 0]:
        ...
        for kmer in [3,4,5,6]:
            ...
            config.output_attentions = True
            model_dir = os.path.join(args.model_name_or_path, str(kmer))
            model = model_class.from_pretrained(
                model_dir,
                from_tf=bool(".ckpt" in args.model_name_or_path),
                config=config,
            ...
            # elif kmer == 5:
            #     args.data_dir = args.data_dir.replace("/" + str(kmer - 2), "/" + str(kmer))
            else:
                args.data_dir = args.data_dir.replace("/"+str(kmer-1), "/"+str(kmer))

            # pred_dir = os.path.join(args.result_dir, str(kmer))
            # if pred_dir.split('/')[-1] == "3":
        ...
        data_path = os.path.join(root_path, "ensemble")
        if not os.path.exists(data_path):
            os.makedirs(data_path)
